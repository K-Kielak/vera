...
def is_fake(url):
    ...
    with tf.Session() as sess:
        ...
        print("prediction:\n", prediction.eval({x: np.array(features).reshape(1, input_size)}))
        classification = tf.argmax(prediction, 1)
        classification = classification.eval({x: np.array(features).reshape(1, input_size)})
...
is_fake('http://conservative101.com/obama-just-launched-crazy-last-ditch-plan-stop-trump-backfired/')